# Anyscale Networking Stack v2 - Minimal Parameters

This **example** will build the resources necessary to run Anyscale in an AWS account. The goal of this example
is to use the fewest possible parameters/variables to create the resources. This example should be considered
a minimal viable example and shouldn't be used for production Anyscale workloads (please look at any of the other
`anyscale-v2` examples).
...
The outputs from this Terraform can be used to register an Anyscale Cloud with the Anyscale CLI. To use:
...
| Name | Description |
|------|-------------|
| <a name="output_anyscale_register_command"></a> [anyscale\_register\_command](#output\_anyscale\_register\_command) | Anyscale register command.<br>This output can be used with the Anyscale CLI to register a new Anyscale Cloud.<br>You need to replace `<CUSTOMER_DEFINED_NAME>` with a name of your choosing. |
