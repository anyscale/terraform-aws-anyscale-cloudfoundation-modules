locals {
  ...
  eks_name_version  = var.include_version_in_name ? "-${replace(var.kubernetes_version, ".", "_")}" : ""
  eks_name_cloud_id = var.anyscale_cloud_id != null && var.anyscale_eks_name == null ? "eks-anyscale-${var.anyscale_cloud_id}${local.eks_name_version}" : null
  eks_name_computed = coalesce(var.anyscale_eks_name, local.eks_name_cloud_id, "eks-anyscale${local.eks_name_version}")
...
resource "aws_eks_cluster" "anyscale_dataplane" {
  ...
  #checkov:skip=CKV_AWS_339:Version is decided by the user
  count = local.module_enabled ? 1 : 0
  name  = local.eks_name_computed
